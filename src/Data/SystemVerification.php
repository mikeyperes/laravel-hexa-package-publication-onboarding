...
final readonly class SystemVerification
{
    ...
    public function __construct(
        ...
        if (! in_array($this->system, ['source', 'destination'], true)) {
            throw new InvalidArgumentException('A verification must target source or destination.');
        }
        if (! $this->success && trim($this->blocker) === '') {
            throw new InvalidArgumentException('A failed verification requires a safe blocker.');
        }
